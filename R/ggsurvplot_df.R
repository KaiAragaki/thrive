...
.check_ylab <- function(ylab, fun){
  if(!is.null(fun) & is.character(fun) & ylab == "Survival probability"){
      ylab <- switch(fun,
...
.get_lty <- function(linetype){
  if (length(linetype) <= 1) {
    return(list(lty = linetype, lty.manual = NULL))
  }

  all_base_lty <- all(linetype %in% c("blank", "solid", "dashed", "dotted", "dotdash", "longdash", "twodash"))

  if (is.numeric(linetype) | all_base_lty) {
    return(list(lty = "strata", lty.manual = linetype))
  }

  list(lty = linetype, lty.manual = NULL)
}
